...
// TODO: Switch to using https://developer.auth0.com/resources/guides/spa/angular/basic-authentication#call-a-protected-api-from-angular
// but for now we still have this class we wrote...
...
export class HttpInterceptorService {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // Otherwise, we check if authenticated, if not, try to auth...
    return this._authService.isAuthenticated$.pipe(
      switchMap((authenticated: boolean) => {
        if (authenticated && this._token.length > 0) {
          ...
          // We need to try to authenticate
