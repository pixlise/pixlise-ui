...
export class BinaryDrawModel implements DrawModelWithPointGroup {
  ...
  regenerate(
    ...
    if (raw) {
      ...
      if (
        raw.yAxisInfo.valueRange.min !== null &&
        raw.yAxisInfo.valueRange.max !== null
      ) {
        this.yValueRange = new MinMax(
          Math.floor(raw.yAxisInfo.valueRange.min),
          this.getAxisMax(raw.yAxisInfo.valueRange.max * 1.1)
