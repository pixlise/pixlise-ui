...
export class RGBUPlotExporter {
  ...
  exportPlotImage(mdl: RGBUPlotModel, includeKey: boolean, darkMode: boolean, exportWidth: number = 800): Observable<string> {
    ...
    return new Observable<string>(observer => {
      let plot = generatePlotImage(this.drawer, this.transform, mdl.keyItems, exportWidth, exportHeight, includeKey, !darkMode);
      if (plot) {
  ...
  getExportOptions(mdl: RGBUPlotModel, scanId: string): WidgetExportDialogData {
    let imageShortName = this.getImageShortName(mdl.imageName);

    return {
      title: "Export RGBU Plot",
      defaultZipName: `${scanId} - ${imageShortName} - RGBU Plot`,
      options: [
        {
          id: "darkMode",
          name: "Dark Mode",
    ...
    return new Observable<WidgetExportData>(observer => {
      if (request.dataProducts) {
        ...
        let requests = [
          ...
          requestPlotImage ? this.exportPlotImage(mdl, showKey, darkMode, 800) : of(null),
          requestLargePlotImage ? this.exportPlotImage(mdl, showKey, darkMode, 1600) : of(null),
        ];
